...
leaflet(eu_combine) %>%
  ...
  addLegend( pal=mypalette, 
             values=~total_cases, 
             opacity=0.9, title = "Total cases(Thousand)",
             position = "bottomleft" )
...
As Ukraine entered April 2021, it experienced a second outbreak, this country was in its third lockdown phase since the COVID-19 pandemic began in spring 2020, and its second lockdown in the space of just three months. Until 12th September 2021, at least 57772 Ukrainians have died as a result of the pandemic. And unfortunately for Ukraine, either the war with Russia or the pandemic, in both case, had cause casualties. Worse yet, the Ukrainian government's response to coronavirus has been a textbook example of failed crisis management.
...
eu_pop <- read_csv("https://raw.githubusercontent.com/owid/covid-19-data/master/public/data/owid-covid-data.csv") %>% 
  ...
  mutate(location = recode(location,"Bosnia and Herzegovina" = "Bosnia and Herz.")) %>% 
  unique()
...
In summary, based on the newest data from Ministry of Health Contact Center, the fatality rate is about 2.3%, recovery rate is about 95%, which is pretty high, the total test were performed about 38702 per day. Although there are still new cases continue to be reported every day and the trend of deaths is increasing, while the overall trend of infection rate seems to be flattening out recently.
